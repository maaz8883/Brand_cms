...
// REQUEST_URI-based routing (matches orbit/hamptonpublisher pattern)
$requestPath = parse_url($_SERVER['REQUEST_URI'] ?? '', PHP_URL_PATH) ?: '';
$requestPath = str_replace('\\', '/', $requestPath);
$brandPos = stripos($requestPath, '/theglobalpublish/');
if ($brandPos === false && preg_match('#/theglobalpublish$#i', $requestPath)) {
    $requestPath .= '/';
    $brandPos = stripos($requestPath, '/theglobalpublish/');
}
...
if ($brandPos !== false) {
    ...
    $base_url .= substr($requestPath, 0, $brandPos + strlen('/theglobalpublish/'));

    $relative = substr($requestPath, $brandPos + strlen('/theglobalpublish/'));
    ...
    // Fallback: detect base from SCRIPT_NAME
    $base_url = $_scheme.'://'.$_SERVER['HTTP_HOST'];
    $scriptName = str_replace('\\', '/', $_SERVER['SCRIPT_NAME'] ?? '');
    $script_dir = str_replace('\\', '/', dirname($scriptName));
    ...
    // Support ?page= routing (when .htaccess rewrites to index.php?page=X)
...
if ($_isLocalhost && stripos($base_url, '/theglobalpublish/') === false) {
    $base_url = $_scheme.'://'.$_SERVER['HTTP_HOST'].'/Brand_cms/public/theglobalpublish/';
}
$base_url = rtrim($base_url, '/').'/';
$blogs_url = rtrim($base_url, '/').'/blogs';
$path_prams = explode('?', str_replace($base_url, '', $current_url));
$prams = ! empty($path_prams[1]) ? $path_prams[1] : '';
$exampted_pages = ['thankyou.php', '404.php', 'enroll-now.php', 'logo.php', 'website.php', 'brief/seo.php', 'smm.php', 'create_link.php', 'link_details.php', 'paynow.php', 'charge.php', 'fail.php', 'confirm_payment.php'];
$exampt_allfiles = ['create_payment.php', 'confirm_payment.php'];
$no = '(656) 222-1814';
$no1 = 'tel:+16562221814';
$no2 = '(656) 222-1815';
$no3 = 'tel:+16562221815';
$add = 'Office address: 400 N Ashley Dr, Suite 1900, Tampa, FL 33602';
$email = 'info@theglobalpublishers.com';
$url = $_isLocalhost ? $base_url : 'https://theglobalpublishers.com/';
$bname = 'The Global Publishers';
$date = date('Y-m-d H:i:s');
$logo = $_isLocalhost ? $base_url.'assets/images/logo.webp' : 'https://theglobalpublishers.com/assets/images/logo.webp';

// $title =  'Welcome To US Logo Designs';
// $keywords =  '';
// $description =  '';
$domainname = 'theglobalpublishers.com';

$error_prefex = "<p class='alert-danger'>";
$error_sufex = '</p>';

$smtp['host'] = 'mail.theglobalpublishers.com';
$smtp['username'] = 'info@theglobalpublishers.com';
// $smtp['password'] = 'qrkx gmkd nvxg htpv';
$smtp['password'] = getenv('SMTP_PASSWORD') ?: '';
$smtp['port'] = '465';

$from_email = 'info@theglobalpublishers.com';
$from_name = $bname;

// $source= $_POST['first_source'];
$source = 'Organic';
// $source= $_SESSION['first_touch']['source'];
$cta_email = 'info@theglobalpublishers.com';
$cc_email = [
